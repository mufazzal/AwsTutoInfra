...
function runPollCommand(pollComand, eventArray, stackName, resolve, reject) {
    ...
    if (pollProcess.stdout && pollProcess.stdout.toString('utf8')) {
        updateEventArray(JSON.parse(pollProcess.stdout.toString('utf8')), 
            pollComand, eventArray, stackName, resolve, reject)
    }
}

function updateEventArray({StackEvents: events}, pollComand, eventArray, stackName, resolve, reject) {
    for(var i = events.length-1;i>=0;i--){
        const ev = events[i];

        const eventStr = `${ev.ResourceType} <> ${ev.LogicalResourceId} <> ${ev.ResourceStatus} <> ${ev.ResourceStatusReason}`
        if(!eventArray.includes(eventStr)) {
            eventArray.push(eventStr)
            console.log(eventStr)
            if(isLastEvent(ev, pollComand, eventArray, stackName, resolve, reject) && pollTimer) {
                clearInterval(pollTimer);
            }
        }
    }
}
function isLastEvent(ev, pollComand, eventArray, stackName, resolve, reject) {
